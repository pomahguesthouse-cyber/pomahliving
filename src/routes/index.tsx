...
import { ArrowRight, Shield, Zap, Droplets, Bot, Cpu, BrainCircuit, Orbit, Hexagon, Wifi, Radio } from "lucide-react";
...
function Landing() {
  return (
    <div className="min-h-screen bg-[#d7d0c7] p-3 sm:p-5 lg:p-7 selection:bg-indigo-500/30">
      <main className="relative min-h-[calc(100vh-24px)] overflow-visible rounded-[28px] border border-white/70 bg-[#02050a] shadow-[0_35px_100px_rgba(15,23,42,0.35)] sm:min-h-[calc(100vh-40px)] sm:rounded-[38px] lg:min-h-[calc(100vh-56px)] lg:rounded-[56px]">
        <header className="sticky top-3 z-50 overflow-hidden rounded-t-[28px] rounded-b-[24px] border-b border-white/[0.08] bg-[#02050a]/82 shadow-[0_18px_60px_rgba(2,5,10,0.38)] backdrop-blur-2xl sm:top-5 sm:rounded-t-[38px] lg:top-7 lg:rounded-t-[56px] lg:rounded-b-[34px]">
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_50%_0%,rgba(99,102,241,0.24),transparent_62%)]" />
          <div className="relative mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8 lg:px-10">
            <Link to="/" className="flex items-center gap-2.5 group">
              <div className="relative"><div className="relative z-10 flex h-9 w-9 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-sm font-bold text-white shadow-lg shadow-indigo-500/30">P</div><div className="absolute -inset-1 rounded-2xl bg-gradient-to-br from-indigo-400 to-purple-600 opacity-30 blur-md transition-all duration-700 group-hover:opacity-60" /></div>
              <span className="font-semibold tracking-tight text-white">pomah<span className="text-indigo-400">/living</span></span>
            </Link>
            <nav className="hidden items-center gap-8 text-sm md:flex">{["Agents", "Stats", "Pricing"].map((item) => <a key={item} href={`#${item.toLowerCase()}`} className="text-white/45 transition-colors hover:text-white/85">{item}</a>)}</nav>
            <Link to="/auth"><Button className="rounded-2xl bg-white text-slate-950 hover:bg-white/90">Get started</Button></Link>
          </div>
        </header>

        <div className="relative overflow-hidden rounded-b-[28px] sm:rounded-b-[38px] lg:rounded-b-[56px]">
          <div className="pointer-events-none absolute inset-0 z-0 rounded-b-[inherit]"><div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_0%,rgba(99,102,241,0.18),transparent_50%)]" /><div className="absolute inset-0 bg-[radial-gradient(ellipse_at_80%_80%,rgba(168,85,247,0.13),transparent_50%)]" /><div className="absolute top-1/3 left-1/4 h-[500px] w-[500px] rounded-full bg-indigo-500/5 blur-[120px]" /><div className="absolute bottom-1/3 right-1/4 h-[400px] w-[400px] rounded-full bg-purple-500/5 blur-[100px]" /><div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: `linear-gradient(rgba(99,102,241,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(99,102,241,0.5) 1px, transparent 1px)`, backgroundSize: '80px 80px' }} /></div>
